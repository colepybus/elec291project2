...
void main (void)
{
	...
	InitPinADC(2, 2); // P2.2 - Vertical
	InitPinADC(2, 3); // P2.3 - Horizontal
	InitPinADC(2, 4); // P2.4
	InitPinADC(2, 5); // P2.5
    InitADC();
	InitPushButton();
	...
	LCD_4BIT();
	LCDprint("Joystick Norm:", 1, 1);
	LCDprint("               ", 2, 1);

	while(1)
	{
		v[0] = Volts_at_Pin(QFP32_MUX_P2_2); // Vertical
		v[1] = Volts_at_Pin(QFP32_MUX_P2_3); // Horizontal

		norm_x = (v[1] / 3.29) * 2.0 - 1.0;
		norm_y = (v[0] / 3.29) * 2.0 - 1.0;

		sprintf(lcd_buff, "X=%+.2f Y=%+.2f", norm_x, norm_y);
		LCDprint(lcd_buff, 1, 1);

		// Optional: show button state
		button_state = (P3 & (1 << 2)) ? 0 : 1;
		if (button_state)
			LCDprint("Button: PRESSED", 2, 1);
		else
			LCDprint("Button: RELEASE", 2, 1);

		waitms(200); // Small delay to prevent LCD flicker
	}
}
